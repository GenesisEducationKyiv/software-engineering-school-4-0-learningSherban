  ...
  async getRate(targetCurrency: string): Promise<number | null> {
    const res = await firstValueFrom(
      this.httpService.get(
        `${this.baseURL}/p24api/pubinfo?json&exchange&coursid=5`,
      ),
    );
    console.log('RES', res);
    // const res = (await this.request(config)) as AxiosResponse;
    console.log(
      res.data.find((el: IPrivatBankCurrency) => el.ccy === targetCurrency),
    );
    return res.data.find((el: IPrivatBankCurrency) => el.ccy === targetCurrency)
      ?.buy;
  }
}
