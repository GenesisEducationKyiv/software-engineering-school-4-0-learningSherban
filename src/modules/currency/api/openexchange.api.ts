...
export class OpenExchangeApi extends BaseAxiosApi implements IExchangeApi {
    constructor(
        ...
        this.authorization = this.dotenvService.getVariable("OPENEXCHANGE_API_KEY") as string;
    }

    async getRate(targetCurrency: string): Promise<number | null> {
        const res = await firstValueFrom(
            this.httpService.get(`${this.baseURL}/api/latest.json?&symbols=${targetCurrency}`, {
                headers: {
